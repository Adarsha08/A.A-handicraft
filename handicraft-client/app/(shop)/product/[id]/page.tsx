...
import { ShoppingBag, Heart, Star, ShieldCheck, Truck, RotateCcw, Share2, Plus, Minus, Maximize2, Loader2 } from 'lucide-react'
  ...
  if (loading) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm font-bold uppercase tracking-widest text-primary-brown/40">Loading...</p>
      </div>
    )
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
          <div className="space-y-6">
            <motion.div
              ...
              className="relative aspect-square rounded-[2rem] overflow-hidden bg-white border border-primary-brown/5 shadow-inner group cursor-zoom-in"
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              <span className="text-accent-gold uppercase tracking-[0.2em] font-bold text-xs">{product.category?.name}</span>
            ...
            <div className="flex items-center space-x-3 p-4 bg-white border border-primary-brown/5 rounded-2xl text-sm">
            ...
            <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 pt-4">
              ...
              <button
                ...
                className="flex-grow bg-primary-brown text-white rounded-full font-bold uppercase tracking-widest py-4 hover:bg-black transition-all flex items-center justify-center space-x-3 shadow-lg"
              >
            ...
            <div className="flex space-x-4 pt-2">
              <a
                href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`Namaste! I'm interested in the "${product.name}". Could you tell me more?`)}`}
                target="_blank"
                rel="noreferrer"
                className="flex-grow border-2 border-accent-gold text-accent-gold rounded-full font-bold uppercase tracking-widest py-4 hover:bg-accent-gold hover:text-white transition-all flex items-center justify-center space-x-3"
              >
                <Share2 size={20} /><span>Inquire on WhatsApp</span>
              </a>
              <button
                onClick={() => toggleWishlist(product)}
                className={`p-4 rounded-full border transition-all duration-300 ${isFavorited ? 'bg-accent-red border-accent-red text-white' : 'border-primary-brown/10 text-accent-red'}`}
              >
