...
import { ArrowRight, ShieldCheck, Truck, Globe, Award, Loader2 } from 'lucide-react'
import { useCategories, useProducts } from '@/hooks/fetchHooks'
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-white">
          ...
          >
            ...
            <p className="text-lg text-white/80 leading-relaxed">
              Discover unique religious items, traditional jewelry, and exquisite wooden crafts meticulously created by veteran artisans from the heart of Nepal.
            </p>
            <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6">
              <Link href="/shop" className="bg-accent-gold hover:bg-yellow-600 text-white px-10 py-4 rounded-full font-bold uppercase tracking-widest transition-all flex items-center justify-center group">
                Shop Collection
...
<section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
  ...
  {categories.length === 0 ? (
    <p className="text-primary-brown/40 text-sm">Loading categories...</p>
  ) : (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
      {categories.map((category, idx) => (
        <Link key={category.id} href={`/shop?category=${encodeURIComponent(category.name)}`} className="group text-center space-y-4">
          <div className="aspect-square bg-white rounded-full p-6 border border-primary-brown/5 shadow-sm group-hover:shadow-md group-hover:border-accent-gold/30 transition-all flex items-center justify-center overflow-hidden">
            <img src={`https://picsum.photos/seed/cat${idx}/200/200`} alt={category.name} className="w-16 h-16 object-contain grayscale group-hover:grayscale-0 transition-all" referrerPolicy="no-referrer" />
          </div>
          <p className="text-sm font-serif font-bold tracking-wide uppercase group-hover:text-accent-gold transition-colors">{category.name}</p>
        </Link>
      ))}
    </div>
  )}
      ...
      <section className="bg-primary-brown py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center space-y-4 mb-16">
            ...
            <p className="text-white/60 max-w-xl mx-auto">Our highly detailed statues and textiles are favored by collectors and spiritual practitioners worldwide.</p>
          </div>
          {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-12">
            <div className="space-y-4">
              ...
              <p className="text-primary-brown/70 leading-relaxed">A.A. HANDICRAFT isn't just a shop; it's a bridge between the ancient stone carvers, metal workers, and weavers of Kathmandu and the world.</p>
            </div>
          ...
          <div className="relative">
            <div className="aspect-[4/5] rounded-[40px] overflow-hidden">
              <img src="https://picsum.photos/seed/artisan/800/1000" alt="Artisan working" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            </div>
      ...
      <section className="bg-accent-gold/10 py-20">
        <div className="max-w-4xl mx-auto px-4 text-center space-y-8">
          ...
          <p className="text-primary-brown/70">Sign up for exclusive previews of new artisan collections and stories from Kathmandu.</p>
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button className="bg-primary-brown text-white px-10 py-4 rounded-full font-bold uppercase tracking-widest hover:bg-black transition-colors">Subscribe</button>
          </div>
        </div>
      </section>
    </div>
